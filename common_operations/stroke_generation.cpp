...
stroke generate_stroke (const stroke_limit_descriptor &stroke_descriptor)
{
	...
	if (!stroke_descriptor.validate_self()) assert(false);
	...
	// Firstly generate the stroke without its position (point0 is (stroke_descriptor.image_rectangle.min_x; stroke_descriptor.image_rectangle.min_y)):
	auto gen_x = [&](){ return stroke_descriptor.image_rectangle.min_x + randval(stroke_descriptor.min_dx, stroke_descriptor.max_dx, gen); };
	auto gen_y = [&](){ return stroke_descriptor.image_rectangle.min_y + randval(stroke_descriptor.min_dy, stroke_descriptor.max_dy, gen); };
...
//	std::cout << max_shift_x << " " << max_shift_y << std::endl;
	...
	double shift_x = randval(0, max_shift_x, gen);
	double shift_y = randval(0, max_shift_y, gen);
