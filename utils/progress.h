...
class Progress {
 private:
  ...
  [[nodiscard]] double get_progress() const { return current / (double) total; }

  [[nodiscard]] double get_time_per_it() const { return std::chrono::duration<double>(t_current - t_start).count() / (current - start); }
 ...
 public:
  ...
  [[nodiscard]] bool IsWorking() const { return is_working; }
