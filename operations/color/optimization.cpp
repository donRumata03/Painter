...
Color find_stroke_color_by_gradient_descent(const Stroke& colorless_stroke, const Image& image, double learning_rate,
  ...
  std::vector<double> initial_color_component_sequence = {initial_color.r, initial_color.g, initial_color.b};
  ...
  auto error_function_gradient_counter = [&](const std::vector<double>& current_color_vector,
                                             double current_error_function = -1.) -> std::vector<double> {
    ...
    return {gradient.begin(), gradient.end()};
  };

  auto error_function_counter = [&](const std::vector<double>& current_color_vector) -> double {
    Color current_color(current_color_vector);
  ...
  auto[optimization_mse, optimization_color_vector] = gradient_optimize(error_function_counter,
                                                                        error_function_gradient_counter,
                                                                        initial_color_component_sequence, learning_rate,
                                                                        iterations); // TODO: use logger if debugging enabled
  ...
  if (log_debug_information) {
    LogInfo("Stroke", "Optimization", "find_stroke_color_by_gradient_descent")
            << "Error for dummy color pick: " << initial_error << ", Error for optimization color pick: "
            << optimization_mse;
  }
...
Color find_stroke_color_by_ariphmetic_mean(const Stroke& colorless_stroke, const Image& image) {
  ...
  return Color(r_sum / points_in_stroke, g_sum / points_in_stroke, b_sum / points_in_stroke);
}
