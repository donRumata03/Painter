...
std::ostream& operator<<(std::ostream& os, const ImageCell& cell) {
  os << "ImageCell { " << "min_y: " << cell.min_y << " max_y: " << cell.max_y
...
ImageZoneRepresentation::ImageZoneRepresentation(std::vector<std::vector<ImageCell>> cells, 
                                                 size_t _image_w, size_t _image_h)
    : cells_2d(std::move(cells)), image_w(_image_w), image_h(_image_h) {}

const std::vector<std::vector<ImageCell>>& ImageZoneRepresentation::get_2d_cells() const {
  return cells_2d;
}
...
size_t ImageZoneRepresentation::get_image_w() const { return image_w; }

size_t ImageZoneRepresentation::get_image_h() const { return image_h; }

size_t ImageZoneRepresentation::get_x_zones() const { return cells_2d.size(); }

size_t ImageZoneRepresentation::get_y_zones() const { return cells_2d[0].size(); }

std::ostream& operator<<(std::ostream& os, const ImageZoneRepresentation& zones) {
  os << "image_w: " << zones.image_w << " image_h: " << zones.image_h << " cells_2d: " << zones.cells_2d;
  return os;
}
...
static ImageZoneRepresentation construct_cells(const std::vector<std::pair<size_t, size_t>>& xs,
  ...
  for (const auto& x : xs) {
    ...
    for (const auto& y : ys) {
      this_col.emplace_back(y.first, y.second, x.first, x.second);                              
    }
  }
...
static void extend_middle_borders(std::vector<std::pair<size_t, size_t>>& values, 
                                  double relative_cell_overlay_size) {
  std::vector<double> cell_sizes;
  cell_sizes.reserve(values.size());

  for (auto& range : values) {
    cell_sizes.push_back(double(range.second - range.first));
  }
...
ImageZoneRepresentation schedule_image_splitting(size_t image_w, size_t image_h, size_t x_cells, 
  ...
  std::vector<std::pair<size_t, size_t >> xs_distribution = distribute_task_ranges(image_w, x_cells),
