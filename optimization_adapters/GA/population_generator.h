...
class GaPopulationGenerator {
 ...
 public:
  ...
  explicit GaPopulationGenerator(const std::vector<Stroke>& strokes) : m_generator(strokes) {}
  ...
  operator()(const std::vector<std::pair<double, double>>& ranges, size_t amount) const {
    ...
    for (size_t image_index = 0; image_index < amount; ++image_index) {
      res[image_index] = m_generator(strokes_in_image * sizeof(Stroke) / sizeof(double));
    }
