...
GaLoggingCallback::GaLoggingCallback(ImageStrokingData imageData, const std::string& path_for_logging,
                                     double logged_percent, bool log_precisely, bool enable_console_output,
                                     Color canvas_color)
        : path_for_logging(path_for_logging), imageData(std::move(imageData)), canvas_color(canvas_color),
          logged_genome_rate(logged_percent), detalized_logging(log_precisely),
...
void GaLoggingCallback::operator()(const GA::Population& population, size_t epoch_index,
  ...
  switch (this_logging_type) {
    case logging_type::new_epoch:
      ...
      operation_index = 1;
      if (enable_console_output) LogInfo("GA", "Log") << "Generated new population...";
      break;
    case logging_type::after_mutation:
      this_operation_name = "after_mutation";
      operation_index = 2;
      if (enable_console_output) LogInfo("GA", "Log") << "Mutated...";
      break;
    case logging_type::after_constraining:
      this_operation_name = "after_constraining";
      operation_index = 3;
      if (enable_console_output) LogInfo("GA", "Log") << "Constrained...";
      break;
    default:
      assert(false);
  }
  ...
  if (enable_console_output) LogInfo("GA", "Log") << "Logging " << genomes_to_log << "genomes";
