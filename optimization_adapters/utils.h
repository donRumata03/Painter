...
class Packer {
 private:
  ...
  static double smooth_transmit_between_borders_tanh(double const max_dimensions,
                                              double max_width);

  static double fatness_to_width(const Stroke& stroke, double fatness);

  static double width_to_fatness(const Stroke& stroke, double width);

 public:
  static CommonStrokingParams params;
  Packer();

  static std::vector<ColoredStroke> unpack_stroke_data_buffer(const std::vector<double>& buffer);
  ...
  static std::vector<double> pack_stroke_data(const std::vector<StrokeType>& strokes) {
